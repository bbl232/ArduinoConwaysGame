...
void loop() {
  ...
  for(int x = 0; x < width; x++){
    for (int y = 0; y < height; y++){
      ...
      if(getCell(cells, x, y) && (neighbours < 2 || neighbours > 3)){
          setCell(cells, x, y, dead);
          drawCell(x,y,BLACK);
      }
      else if(!getCell(cells, x, y) && (neighbours == 3)){
        setCell(cells, x, y, alive);
        drawCell(x,y,WHITE);
      }
    }
  }
